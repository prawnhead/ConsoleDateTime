...
void testAddOneDay(DateTime alpha, DateTime omega) {
  tests++;
  DateTime test = DateTime(alpha);
  test.addOneDay();
  if (test != omega) {
    fails++;
    printf("FAILURE of addOneDay() %s.\n\tExpected: %s. Actual: %s.\n", alpha.toString().c_str(), omega.toString().c_str(), test.toString().c_str());
  }
}
